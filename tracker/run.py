...
def main(argv: list[str] | None = None) -> int:
    ...
    p.add_argument("--mode", choices=["preopen", "postclose"], required=True)
    p.add_argument("--out", type=Path, default=DEFAULT_OUT)
    p.add_argument("--pretty", action="store_true", help="indent the JSON output")
    args = p.parse_args(argv)

    snap = build_snapshot(args.mode)
    ...
    store.write_snapshot(snap, args.mode)

    n = len(snap.get("tickers", []))
    pf = snap.get("portfolio", {})
    print(f"[{args.mode}] {store.backend()} · {n} tickers · "
